...
async function create(): Promise<void> {
  ...
  saving.value = true
  try {
    const definition = createProcessActivity(title.value.trim())
    definition.metadata.description = description.value.trim()
    await activities.save(definition)
    const id = activities.current?.id
    if (!id) {
      throw new Error('Process was created but could not be loaded')
    }
    await router.push(`/studio/process/${id}`)
  } catch (cause) {
    window.alert(cause instanceof Error ? cause.message : 'Failed to create process')
  } finally {
    saving.value = false
  }
}
    ...
    <div class="author-page-inner author-stack">
      <div class="author-header-row">
        <div class="author-header-left">
          ...
          <h1 class="author-header-title">New Process</h1>
        </div>
      </div>
