...
async function create(): Promise<void> {
  ...
  saving.value = true
  try {
    const definition = createLessonActivity(title.value.trim())
    ...
    await router.push(`/studio/lesson/${id}`)
  } catch (cause) {
    window.alert(cause instanceof Error ? cause.message : 'Failed to create lesson')
  } finally {
    saving.value = false
  }
}
    ...
    <div class="author-page-inner author-stack">
      <div class="author-header-row">
        <div class="author-header-left">
          ...
          <h1 class="author-header-title">New Lesson</h1>
        </div>
      </div>
